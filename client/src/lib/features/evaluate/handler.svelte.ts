...
export class EvaluationHandler {
	reponse = $state<Task2EvaluationResponse | undefined>();
	isExecuting = $state(false);
	onError: ((errorMessage: string) => void) | undefined = undefined;

	#api = new EvaluationApi();

	async execute(topic: string, response: string) {
		try {
			...
			const stream = await this.#api.send({ topic, response_text: response });
			...
			parser.onValue = ({ key, stack, value }) => {
				if (!this.reponse || !value) return;
				if (stack.length === 1 && key === "overall_band") {
					this.reponse.overall_band = value as Task2EvaluationResponse["overall_band"];
				} else if (stack.length === 2 && stack[1]?.key === "criteria" && isCriteriaKey(key)) {
					this.reponse.criteria[key] = value as Task2EvaluationCriterion;
				}
			};
			...
			while (true) {
				...
				if (done || value?.data === "[DONE]") {
					if (!this.reponse.overall_band) throw new Error("overall_band missing");
					break;
				}
