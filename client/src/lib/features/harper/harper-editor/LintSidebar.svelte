<script lang="ts">
	...
	interface Props {
		...
		activeLintId?: string | null;
		focusLint?: (lintBox: IgnorableLintBox) => void;
		onActivate?: (lintBox: IgnorableLintBox | null) => void;
		onApplied?: () => void;
		onIgnored?: () => void;
		onIgnoreAll?: (() => void | Promise<void>) | null;
		onHideSidebar?: () => void;
	}
	...
	let problemCountLabel = $derived(
		`${lintBoxes.length} ${lintBoxes.length === 1 ? "problem" : "problems"}`
	);
	let counts = $derived(
		lintBoxes.reduce((acc, lintBox) => {
			acc[lintBox.lint.lint_kind] += 1;
			return acc;
		}, createEmptyLintKindCounts())
	);
	let visibleLintKindEntries = $derived(LINT_KIND_STYLE_ENTRIES.filter(([key]) => counts[key] > 0));
	let signature = $derived(lintBoxes.map((lintBox) => lintBox.lint.context_hash).join("|"));
	onMount(() => {
...
<aside
	class="flex min-h-0 w-[320px] flex-[0_0_320px] overflow-hidden border-t border-l border-base-content/20 bg-base-100 text-base-content @max-[760px]:w-full @max-[760px]:flex-[0_0_42%] @max-[760px]:border-t-[0.5px] @max-[760px]:border-l-0"
	aria-label="Problems"
	...
	<div class="flex min-h-0 flex-1 flex-col" transition:fade={{ duration: 250 }}>
		<header class="flex items-center gap-1.5 px-3.5 pt-2.5 pb-2">
			<h2
				class="m-0! flex min-w-0 flex-1 items-center p-0! font-[inherit]! text-[15px]! leading-none! font-bold!">
				<button
					type="button"
					class="m-0! inline-flex min-w-0 items-center gap-1.5 border-0 bg-transparent p-0! text-left font-[inherit] text-[15px]! leading-none! text-inherit"
					aria-expanded={showLintKindCounts}
					onclick={() => (showLintKindCounts = !showLintKindCounts)}>
					Problems
					<span
						class="inline-flex h-4.5 min-w-5 items-center justify-center rounded-full bg-primary px-1.5 text-[11px] font-semibold text-primary-content tabular-nums">
						{lintBoxes.length}
					</span>
					<span
		...
		{#if showLintKindCounts && visibleLintKindEntries.length > 0}
			<div
				class="grid grid-cols-3 gap-x-2 gap-y-1.75 overflow-hidden border-b-[0.5px] border-[rgba(28,26,22,0.09)] px-4.5 pb-3"
				aria-label="Problem lint kinds">
				{#each visibleLintKindEntries as [key, lintKindStyle] (key)}
					<div
						class="grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-1 text-[10px] font-medium whitespace-nowrap text-base-content">
						<span
							class={`inline-flex h-2.25 w-2.25 shrink-0 items-center justify-center rounded-full ${lintKindStyle.haloClass}`}>
						...
						<strong class="font-medium text-base-content tabular-nums">{counts[key]}</strong>
					</div>
				{/each}
			</div>
		{/if}
		...
		<div
			class="flex min-h-0 flex-1 flex-col gap-2 overflow-auto px-3.5 pt-3.5 pb-6"
			data-problems-scroller>
			{#if lintBoxes.length === 0}
				<div
					class="m-auto max-w-55 px-3 py-7 text-center text-[12.5px] leading-[1.55] text-base-content">
					<div
						aria-hidden="true"
...
{#if showIgnoreConfirm}
	<div
		...
		onclick={handleIgnoreBackdropClick}>
		<div
			class="w-full max-w-85 rounded-lg border-[0.5px] border-[rgba(28,26,22,0.16)] bg-white p-4 text-stone-950 shadow-2xl shadow-stone-950/20"
			role="dialog"
			aria-modal="true"
