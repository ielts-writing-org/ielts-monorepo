...
import { type LintKind, lintKindColor, lintKindTextColor } from "./lint-lind-color";
import { BookDown, SlidersVertical, SquareOff } from "@lucide/svelte";
import type { UnpackedLint, UnpackedSuggestion } from "./unpack-lint";
...
const autofocusedNodes = new WeakSet<HTMLElement>();

const createFocusHook = () => ({
	hook: (node: HTMLElement) => {
		if (autofocusedNodes.has(node)) {
			return;
		}
...
function body(message_html: string) {
...
function hintDrawer(hint: string | null) {
...
function addToDictionary(box: LintBox, addToUserDictionary?: (words: string[]) => Promise<void>) {
	return h(
		"button",
		{
			className: "harper-btn",
...
function ignoreLint(onIgnore: () => void | Promise<void>) {
	return button(
		"Dismiss",
